...
use crate::{config::MqttConfig, wol::WolConfig};
...
pub async fn init_mqtt(mqtt_config: &MqttConfig, wol_config: &WolConfig) -> Result<MqttClient> {
    ...
    for device in wol_config.devices.values() {
                ...
                let res = (|| async move {
                    match notification? {
                        rumqttc::Event::Incoming(rumqttc::Packet::ConnAck(_)) => {
                            client
                                .subscribe("home/lights/wol/+/set", QoS::AtMostOnce)
                                .await?;
                        }
                        rumqttc::Event::Incoming(rumqttc::Packet::Publish(msg)) => {
                            ...
                            let tx = mqtt_tx.get(device_id).with_context(|| {
                                format!(
